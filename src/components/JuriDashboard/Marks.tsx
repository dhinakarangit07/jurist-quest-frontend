// app/tournaments/jessup-2025/marks-entry/page.jsx
"use client"
...
import { Calendar, Clock, Users, Star, MapPin, Gavel, Save, RefreshCw } from "lucide-react"
...
const MarksEntryPage = () => {
  ...
  const calculateTotal = (scores) => {
    return Object.values(scores).reduce((total, criterion) => {
      const mainPoints = parseFloat(criterion.points) || 0;
      const subPoints = Object.values(criterion.sub_scores).reduce((subTotal, sub) => subTotal + (parseFloat(sub.points) || 0), 0);
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="bg-gradient-to-r from-[#2d4817] to-[#3d5a1f] rounded-lg p-6 text-white">
        ...
        <p className="text-sm opacity-75">{matchToScore.match_name} - {matchToScore.case_details.case_title}</p>
      </div>
              ...
              <div className="mt-2 text-sm">
                ...
                <ul className="list-disc list-inside">
                  {matchToScore.applicant_team.participants.map((member, idx) => (
                    <li key={idx} className="text-gray-600">{member.name} ({member.role})</li>
                  ))}
                </ul>
              </div>
            </div>
                      ...
                      <div key={`applicant-${criterion.id}-${sub.id}`} className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            onChange={(e) => handleScoreChange('applicant', criterion.id, sub.id, 'points', e.target.value)}
                          />
                      ...
                      <div key={`respondent-${criterion.id}-${sub.id}`} className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            onChange={(e) => handleScoreChange('respondent', criterion.id, sub.id, 'points', e.target.value)}
                          />
                          <span className="text-xs text-gray-500">/ {sub.max_points}</span>
                        </div>
                      </div>
